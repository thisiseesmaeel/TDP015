...
def make_images():
    ...
    reader = {d: [] for d in range(10)}
    for image, digit in mnist.read_training_data():
            reader[digit].append(image)

    for digit in reader:
        n_image = ()
        img = random.choice(reader[digit])
        for x in img:
            if x == 1:
                n_image += (0,)
            else:
                n_image += (255,)
        png.save_png(28, 28, n_image, 'digit-{}.png'.format(digit))
